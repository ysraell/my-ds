...
CI_REGISTRY_PATH=`settings2env CI_REGISTRY_PATH ${EXT_SETTINGS}`

docker tag $APP_NAME:$TAG  $CI_REGISTRY/$CI_REGISTRY_PATH/$APP_NAME:$CI_COMMIT_TAG
...
docker push $CI_REGISTRY/$CI_REGISTRY_PATH/$APP_NAME:$CI_COMMIT_TAG
...
cat base_docker-compose.yml \
    ...
    |sed -e "s/CI_REGISTRY_PATH/${CI_REGISTRY_PATH}/g" \
